...
/**
 * Calculate the 100-Point Composite Oculus Score and breakdown for a guard
 */
export function calculateOculusScore(params: {
  guardId: string;
  ...
  // -------------------------------------------------------------
  // 3. Composite Oculus Score (0 - 100 Pts)
  // -------------------------------------------------------------
  const oculusScore = Math.max(0, Math.min(100, Math.round(operationalReliabilityScore + clientExperienceScore)));
  ...
  } else if (oculusScore >= 88) {
    tier = 'gold';
    tierLabel = 'Master Gold Tier';
  } else if (oculusScore >= 80) {
    tier = 'silver';
    tierLabel = 'Proficient Silver Tier';
  } else if (oculusScore >= 70) {
    tier = 'bronze';
