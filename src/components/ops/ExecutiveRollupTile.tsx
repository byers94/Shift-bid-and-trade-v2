...
export interface ExecutiveRollupTileProps {
  variant?: 'all' | 'metrics' | 'contracts';
}

export const ExecutiveRollupTile: React.FC<ExecutiveRollupTileProps> = ({
  variant = 'all'
}) => {
  const { 
    standardReports, 
  ...
  const showMetrics = variant === 'all' || variant === 'metrics';
  const showContracts = variant === 'all' || variant === 'contracts';
      ...
      <div className="p-4 sm:p-5 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between bg-slate-50/50 dark:bg-slate-900/50">
        <div className="flex items-center space-x-3">
          <div className={`p-2 rounded-xl ${
            ...
            {variant === 'contracts' ? <Building2 className="w-5 h-5" /> : <BarChart3 className="w-5 h-5" />}
          </div>
          <div>
            <h3 className="font-bold text-slate-900 dark:text-white text-base">
              {variant === 'contracts' 
                ? 'Contract Expirations & Lifecycle Watch' 
                : variant === 'metrics' 
                  ? 'Executive 48-Hour Rollup & ASR Metrics'
                  : 'Executive 48-Hour Rollup & Facility Watch'}
            </h3>
            <p className="text-xs text-slate-500 dark:text-slate-400">
              {variant === 'contracts'
                ? 'Facility contract expiration warnings, renewal dates, and post commitments'
        ...
        <span className={`px-2.5 py-1 text-xs font-bold rounded-lg border ${
          variant === 'contracts'
            ? 'bg-amber-50 dark:bg-amber-950/40 text-amber-700 dark:text-amber-300 border-amber-200/70 dark:border-amber-800'
            : 'bg-indigo-50 dark:bg-indigo-950/40 text-indigo-700 dark:text-indigo-300 border-indigo-200/70 dark:border-indigo-800'
        }`}>
          {variant === 'contracts' ? `${expiringSoonSites.length} Due Renewal` : 'Executive SLA'}
        </span>
      </div>
